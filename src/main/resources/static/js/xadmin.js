...
layui.use(['layer','element','jquery'],function() {
    ...
    function baseAjax(requestPath, requestData, requestType, succCallback, dataType) {
        /*requestPath：请求路径
        ...
        $.ajax({
            ...
            beforeSend: function (request) {
                //发送请求之前，插入加载提示信息“拼命加载中···”
                request.setRequestHeader("Authorization", sessionStorage.getItem("token"));
            },
            success: function (res) {         //请求成功
                if (res.code == '1000') {   //res.message不是唯一，也有可能是res.code 需结合项目实际场景来写入判断条件
                    if (succCallback) {
                        succCallback(res.data)  //返回OK回调函数，将返回的数据res传入到该回调函数中
                    }
                } else {
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                toastr.error("系统异常，请稍后重试");                     //请求错误，弹出提示
    ...
    $.jPost = function (path, data, succCallback) {
        //再次封装-有参数
        baseAjax(path, data, 'POST', succCallback)
    }

    $.jGet = function (path, data, succCallback) {
        //再次封装-有参数
        baseAjax(path, data, 'GET', succCallback)
    }

    $.jPut = function (path, data, succCallback) {
        //再次封装-有参数
        baseAjax(path, data, 'PUT', succCallback)
    }

    $.jDel = function (path, data, succCallback) {
        //再次封装-有参数
        baseAjax(path, data, 'DELETE', succCallback)
    }
    // 页面加载完要做的
    xadmin.end();
})
